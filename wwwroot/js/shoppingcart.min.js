...
function addToCart(menuitemHTMLElement) {
    const menuitemElement = $(itemHTMLElement);
...
function setInitialQuantityAndTotalPrice(Id) {
    const cart = getCart();
    const i = indexOfMenuItemInCart(cart, itemId);
    if (i >= 0) {
...
function getCart() {
    return JSON.parse(localStorage.getMenuItem("cart")) || [];
}
...
function updateCart(cart) {
    localStorage.setMenuItem("cart", JSON.stringify(cart));
}
